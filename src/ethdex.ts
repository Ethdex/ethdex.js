...
export class ZeroEx {
     ...
     * @param   order   An object that conforms to the Order or SignedOrder or SignedRelayOrder interface definitions.
     * @return  The resulting orderHash from hashing the supplied order.
     */
    public static getOrderHashHex(order: Order|SignedOrder|SignedRelayOrder): string {
    ...
    /**
     * Computes the orderSignedHash for a supplied order.
     * @param   order   An object that conforms to the Order or SignedOrder or SignedRelayOrder interface definitions.
     * @return  The resulting orderSignedHash from hashing the supplied order.
     */
    public static getOrderSignedHashHex(order: Order|SignedOrder|SignedRelayOrder): string {
        assert.doesConformToSchema('order', order, schemas.orderSchema);
        const orderSignedHashHex = utils.getOrderSignedHashHex(order);
        return orderSignedHashHex;
    }
    /**
    ...
    /**
     * Signs an orderSignedHash and returns it's elliptic curve signature.
     * This method currently supports TestRPC, Geth and Parity above and below V1.6.6
     * @param   orderSignedHash Hex encoded orderHash to sign.
     * @param   signerAddress   The hex encoded Ethereum address you wish to sign it with. This address
     *          must be available via the Web3.Provider supplied to 0x.js.
     * @return  An object containing the Elliptic curve signature parameters generated by signing the orderSignedHash.
     */
    public async signOrderHashAsync(orderSignedHash: string, signerAddress: string): Promise<ECSignature> {
        assert.isHexString('orderSignedHash', orderSignedHash);
        ...
        if (isParityNode || isTestRpc) {
            ...
            msgHashHex = orderSignedHash;
        } else {
            const orderHashBuff = ethUtil.toBuffer(orderSignedHash);
        ...
        if (_.includes(validVParamValues, ecSignatureVRS.v)) {
            const isValidVRSSignature = ZeroEx.isValidSignature(orderSignedHash, ecSignatureVRS, signerAddress);
        ...
        if (_.includes(validVParamValues, ecSignatureRSV.v)) {
            const isValidRSVSignature = ZeroEx.isValidSignature(orderSignedHash, ecSignatureRSV, signerAddress);
